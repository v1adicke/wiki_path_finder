...
from benchmarking.dashboard import render_dashboard
...
from benchmarking.version import BENCHMARK_VERSION
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--logo", type=str, default="assets/logo.png", help="Путь к логотипу")
    parser.add_argument("--out-image", type=str, default="reports/dashboard.png", help="Путь PNG-дашборда")
    parser.add_argument("--out-json", type=str, default="reports/metrics.json", help="Путь JSON с метриками")
    return parser.parse_args()


async def run_service(config: BenchmarkConfig, seed: int, logo: Path, out_image: Path, out_json: Path) -> None:
    ...
    render_dashboard(summary=summary, output_path=out_image, logo_path=logo, version=BENCHMARK_VERSION)
